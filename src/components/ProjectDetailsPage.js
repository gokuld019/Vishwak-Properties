...
const isPlotProject = Boolean(
 ...
 const getFilteredTabs = () => {
  ...
  const handlePaymentEnquirySubmit = async (e) => {
    ...
    try {
      const res = await axios.post("http://localhost:5000/api/payment-enquiry", {
        projectId,
  ...
  useEffect(() => {
    ...
    const loadBrochure = async () => {
      const res = await fetch(`http://localhost:5000/api/brochure/${projectId}`);
      const data = await res.json();

      if (data.fileUrl) {
        setBrochureUrl(`http://localhost:5000${data.fileUrl}`);
      }

      if (data.thumbnailUrl) {
        setThumbnailUrl(`http://localhost:5000${data.thumbnailUrl}`);
      }
    };
  ...
  useEffect(() => {
    ...
    const fetchPriceList = async () => {
      try {
        ...
        const res = await fetch(`http://localhost:5000/api/pricelist/${projectId}`);
        const data = await res.json();
    ...
    const fetchMedia = async () => {
      try {
        const res = await axios.get(
          `http://localhost:5000/api/project-media/${projectId}`
        );
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const res = await fetch(`http://localhost:5000/api/project-stats/${projectId}`);
        const data = await res.json();
        ...
        if (data.stats) {
          ...
          setStats([
            ...
            { label: "SALEABLE AREA", value: data.stats.saleableArea, sub: "Area" },
            { label: "NO. OF FLOORS", value: data.stats.floors, sub: "Floors" },
          ]);
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const fetchGallery = async () => {
      try {
        const res = await fetch(`http://localhost:5000/api/gallery/${projectId}`);
        const data = await res.json();
    ...
    async function fetchSmartInvestment() {
      try {
        const res = await fetch(
          `http://localhost:5000/api/smart-investment/${projectId}`
        );
  ...
  useEffect(() => {
    ...
    const fetchPaymentPlan = async () => {
      try {
        const res = await fetch(`http://localhost:5000/api/payment-plan/${projectId}`);
        const data = await res.json();
  ...
  useEffect(() => {
    ...
    const fetchFloorPlans = async () => {
      try {
        const res = await fetch(`http://localhost:5000/api/floorplans/${projectId}`);
        const data = await res.json();
        setFloorPlans(data || []);
  ...
  useEffect(() => {
    ...
    const fetchConstructionUpdates = async () => {
      try {
        const res = await fetch(
          `http://localhost:5000/api/construction-updates/${projectId}`
        );
        const data = await res.json();
  ...
  useEffect(() => {
    ...
    const fetchAll = async () => {
      try {
        ...
        // ---- PROJECT DETAILS ----
        const detailsRes = await fetch(`http://localhost:5000/api/project-details/${numericId}`);
        ...
        const [whyRes, locationRes] = await Promise.all([
          fetch(`http://localhost:5000/api/why/${numericId}`),
          fetch(`http://localhost:5000/api/location/${numericId}`),
        ]);
  ...
  useEffect(() => {
    ...
    const fetchSpecifications = async () => {
      try {
        const res = await fetch(
          `http://localhost:5000/api/specifications/${projectId}`
        );
        const data = await res.json();
  ...
  const handleEnquirySubmit = async (e) => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/floorplans/enquiry", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
...
const desktopHero =
  projectData?.heroImageDesktop
    ? `http://localhost:5000${projectData.heroImageDesktop}`
    : "";

const mobileHero =
  projectData?.heroImageMobile
    ? `http://localhost:5000${projectData.heroImageMobile}`
    : desktopHero;
                ...
                {[
                  {
                    img: projectData?.image1
                      ? `http://localhost:5000${projectData.image1}`
                      : "/interior1.webp",
                    label: "DELIVERING ONLY<br/>THE BEST",
                  },
                  {
                    img: projectData?.image2
                      ? `http://localhost:5000${projectData.image2}`
                      : "/interior2.webp",
                    label: "AFFORDABLE<br/>HOMES",
                  },
                  {
                    img: projectData?.image3
                      ? `http://localhost:5000${projectData.image3}`
                      : "/interior3.webp",
                    label: "TOP CLASS<br/>AMENITIES",
                  },
                  {
                    img: projectData?.image4
                      ? `http://localhost:5000${projectData.image4}`
                      : "/interior4.webp",
                        ...
                        src={
                          plan.image
                            ? plan.image.startsWith("http")
                              ? plan.image
                              : `http://localhost:5000/${plan.image}`
                            : "/floorplan.webp"
                        }
                            ...
                            >
                              ...
                              <button
                                onClick={() => {
                                  ...
                                  setShowPaymentEnquiry(true);
                                }}
                                className="bg-yellow-500 text-gray-900 px-3 py-2 rounded-lg font-semibold hover:bg-yellow-600"
                              >
                                Enquire
                              </button>
                            </div>
      ...
      <section className="w-full bg-white py-12 sm:py-16">
        ...
        <div className="max-w-[1200px] mx-auto px-4 sm:px-6">
          <div className="relative w-full h-[35vh] sm:h-[45vh] md:h-[55vh] lg:h-[65vh]
            ...
            <img
              src={
                galleryImages.length > 0
                  ? `http://localhost:5000${galleryImages[current]?.image}`
                  : images[current]
              }
            ...
            <button
              onClick={handlePrev}
              className="absolute left-2 sm:left-6 top-1/2 -translate-y-1/2 bg-black/50 text-white p-2 sm:p-3 rounded-full hover:bg-black/70 transition-all"
            >
              ←
            </button>
            ...
            <button
              onClick={handleNext}
              className="absolute right-2 sm:right-6 top-1/2 -translate-y-1/2 bg-black/50 text-white p-2 sm:p-3 rounded-full hover:bg-black/70 transition-all"
            >
              →
            </button>
          </div>
                  ...
                  src={
                    selectedFloorPlan.image
                      ? selectedFloorPlan.image.startsWith("http")
                        ? selectedFloorPlan.image
                        : `http://localhost:5000/${selectedFloorPlan.image}`
                      : selectedFloorPlan.image
                  }
          ...
          <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-xl">
            ...
            <form
              onSubmit={async (e) => {
                ...
                const res = await fetch(
                  "http://localhost:5000/api/payment-enquiry",
                  {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
                  ...
                  <div className="mt-4 sm:mt-6 pt-4 sm:pt-6 border-t border-gray-200">
                    ...
                    <div className="flex flex-col sm:flex-row items-center justify-center gap-3 text-sm">
                      ...
                      <a
                        ...
                        className="text-[#67a139] hover:underline font-medium"
                      >
